...
`notion-move-tactic-stack` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies tactic behavior through windowed input fixtures, with late-data behavior checks and explicit failure cases.
...
The score is deterministic. The current threshold is 176. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 5.
